...
setNames(usgs_hydro,c("agency","site_no","date","flowrate (cfs)","code"))

```

This project includes five study sites. This code chunck adds the stage (both ft and cm) for each site based on the flowrate and previously developed regression equations for each site.
```{r}

stage_s1 <- data.frame(0.0011*usgs_hydro[4])
stage_s1 <- setNames(stage_s1, "stage (ft)")

stage_s2 <- data.frame(0.0011*usgs_hydro[4])
stage_s2 <- setNames(stage_s2, "stage (ft)")

stage_s3 <- data.frame(0.056*(usgs_hydro[4]^0.5612))
stage_s3 <- setNames(stage_s3,"stage (ft)")

stage_s4 <- data.frame(0.0014*(usgs_hydro[4]^0.99))
stage_s4 <- setNames(stage_s4, "stage (ft)")

stage_s5 <- data.frame(0.1123*(usgs_hydro[4]^0.4963))
stage_s5 <- setNames(stage_s5, "stage (ft)")
...
stage_cm_s4 <- data.frame(stage_s4[1]*30.48)
stage_cm_s4 <- setNames(stage_cm_s4,"stage (cm)")

stage_cm_s5 <- data.frame(stage_s5[1]*30.48)
stage_cm_s5 <- setNames(stage_cm_s5,"stage (cm)")
```
...
```{r}
# average_natural_stage_14_day <-sum(stage_cm_s1[1:14,1])/14
# recession_rate_natural_14_day <- (stage_cm_s1[1,1] - stage_cm_s1[14,1])/14

recessionrate_14d<- data.frame()

for (i in 1:(nrow(stage_cm_s1))) 
  {recessionrate_14d <- rbind(recessionrate_14d,c((stage_cm_s1[i,1]-stage_cm_s1[13+i,1])/14,
                                                  (stage_cm_s1[i,2]-stage_cm_s1[13+i,2])/14,
                                                  (stage_cm_s1[i,3]-stage_cm_s1[13+i,3])/14))}

recessionrate_14d <- cbind(usgs_hydro[,3],recessionrate_14d)
recessionrate_14d <- setNames(recessionrate_14d,c("date","recessionrate (cm/d)"))
...
```{r}
q_bin_usgs <-cbind(usgs_hydro[3],q_bin_1 <- usgs_hydro[4]<= q_bin[1],
...
q_bin_usgs$date<-as.Date(q_bin_usgs$date, format="%Y/%m/%d")
...
recessionrate_14d <- setNames(recessionrate_14d,c("date","natural recessionrate (cm/d)","scenario 1 recessionrate (cm/d)","scenario 2 recessionrate (cm/d)"))
recessionrate_14d<-na.omit(recessionrate_14d)
```
