...
```{r}
nodes <- c("TIMING","INUN","RECESSION_RATE","HYDRO")
...
#### CPT Assignments ####
The unconditional and conditional probability tables are generated below. The unconditional probability table (upt) for the root node, TIMING, was developed by....????? (scientific literature and expert knowledge/consultation). The CPTs for each node must share the levels of its parents, it must also have a CPT for each of its parent nodes.  

cpt_HYDRO was populated using the expert knowledge informed probabilities from 2014 Morrison & Stone
...
example - https://stackoverflow.com/questions/40364458/r-some-conditional-probability-distributions-of-node-do-not-sum-to-one

bn.fit class structure - http://www.bnlearn.com/documentation/man/bn.fit.class.html

bn.fit source code - https://rdrr.io/cran/bnlearn/src/R/sanitization-fitted.R

custom.fit source code - https://github.com/jtrecenti/bnlearn/blob/master/R/custom.fit.R
