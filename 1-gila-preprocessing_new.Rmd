...
```{r}
usgs_hydro <- read.csv("data/usgs_09430500_daily.txt", skip=28,header = TRUE, sep = "")
...
The hydrologic historical data for the Gila River is located in scenario_revised.csv and includes the 1-natural, 2-Alt. E, 3-Alt.B , and 4-Alt.A (no action)). This code chunk loads scenario_revised.csv (excluding the flow columns for downstream at Riverside) and uses it to create a dataframe called hydro.

```{r}
hydro <- read.csv("data/scenarios_revised.csv", skip=0,header = TRUE)[ ,1:5]
...
###TEST####
# ```{r}
# hydro <- cbind(hydro[1:2],hydro[3]/2,hydro[4]/1,hydro[5])
# ```

This project includes three study sites. This code chunck adds the stage (both ft and cm) for each site based on the flowrate and previously developed regression equations for each site (1-North,2- Middle,3- South). The hydro column called must correspond with each scenario (i.e.1-natural, 2-Alt. E, 3-Alt.B , and 4-Alt.A (no action)).
...
<!-- head_conv <- setNames(head_conv, c("date","natural(ft)", "Alt_E(ft)","Alt_B(ft)","Alt_A(ft)")) -->

<!-- head_cm <- data.frame(hydro$date,head_conv[2]*30.48,head_conv[3]*30.48,head_conv[4]*30.48,head_conv[5]*30.48) -->
<!-- head_cm <- setNames(head_cm,c("date","natural(cm)", "Alt_E(cm)","Alt_B(cm)","Alt_A(cm)")) -->
<!-- ``` -->
...
rownames(rr_14d_prob_yr) <- c("natural(cm/d)","Alt_E(cm/d)", "Alt_B(cm/d)", "Alt_A(cm/d)")
...
Calculates the annual probability of a bin flood event occuring for all four flow scenarios(i.e.1-natural, 2-Alt. E, 3-Alt.B , and 4-Alt.A (no action)).
```{r}
...
row.names(q_prob_yr) <- c("natural", "Alt_E", "Alt_B", "scenario3")
